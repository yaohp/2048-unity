...
public class TileAnimationHandler : MonoBehaviour {
  ...
  private IEnumerator AnimationEntry() {
    ...
    while (_transform.localScale.x < 1/2f) {
      _transform.localScale = Vector3.MoveTowards(_transform.localScale, Vector3.one/2, scaleSpeed * Time.deltaTime);
      yield return null;
    }
  }
  ...
  private IEnumerator AnimationUpgrade() {
    ...
    while (_transform.localScale.x < 1/2f + growSize) {
      _transform.localScale = Vector3.MoveTowards(_transform.localScale, Vector3.one/2 + growVector, scaleSpeed * Time.deltaTime);
      yield return null;
    }
